...
export function app() {
  ...
  const distFolder = join(process.cwd(), 'dist/HomesFromArchitectsWebPage-frontend/browser');
  const imagesFolder = join(process.cwd(), 'dist/HomesFromArchitectsWebPage-frontend/browser/assets/images');
  const indexHtml = existsSync(join(distFolder, 'index.original.html')) ? 'index.original.html' : 'index';
  // pre testovanie v postmanovi
  ...
  server.get('*', (req: express.Request, res: express.Response) => {
    ...
    if (isbot(req.header('User-Agent'))) {
      ...
      // res.sendFile(path.join(__dirname, 'dist/HomesFromArchitectsWebPage-frontend/browser'));
    }
  });
